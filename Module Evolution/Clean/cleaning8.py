...
fips_df = pd.read_csv('/N/u/nickoshy/Carbonate/Sherlock/sherlock-project-master/QGD/county2.csv')
...
def clean(df, seed):
    ...
    def get_data_types(df):
        ...
        for col in df.columns:
            ...
            if date_count > 0:
                ...
                if fips_percentage == 1.0:
                    data_types[col] = 'address'
            else:
                data_types[col] = 'object'

        return pd.DataFrame(data_types.items(), columns=['Column', 'Data_Type'])
    ...
    date_cols = [col for col in data_types_df['Column'] if data_types_df.loc[data_types_df['Column'] == col, 'Data_Type'].values[0] == 'datetime']
    ...
    df = pd.concat([df_numeric, df_object, df_date, df_address], axis=1)
