...
tract_df = pd.read_csv('Indiana_FIPS2.csv')
tract_list = tract_df['tractcode'].drop_duplicates().tolist()
#county_list2 = fips_df['NAME'].tolist()
...
def get_data_types_df(df):
    ...
    for col in df.columns:
        ...
        if year_count / total_count > 0.95:  # If more than 95% of values match year data, classify as year
        ...
        elif tract_count / total_count > 0.95:
            data_types[col] = 'area'
            ...
            tract_count = sum(df[col].apply(lambda x: x in tract_list if pd.notnull(x) else False))
            tract_percentage = tract_count / len(df[col])
            if tract_percentage > 0.95:
                data_types[col] = 'area'
        else:
