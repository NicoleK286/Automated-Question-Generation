...
import datetime
import pandas as pd
from cleaning13 import clean_column_names
from cleaning13 import get_data_types_df # get data types
from cleaning13 import clean # cleaning file
...
from questn14 import ques   # question generation 
from sql_3vp import sqlqueryv3     #SQL query generation 
from questn_3vp import quesv3   # question generation 
import random
from para7 import *
...
# Initializing files
#filename = f'{program_path}sherlock-project-master/QGD/Main3 Running Examples/FIPS Ex5/datafips5.csv'
#datad = f'{program_path}sherlock-project-master/QGD/Main3 Running Examples/FIPS Ex5/dataDescriptionfips5.txt'
#cold = f'{program_path}sherlock-project-master/QGD/Main3 Running Examples/FIPS Ex5/dataColumnfips5.csv'


#Initializing files
#filename = f'{program_path}sherlock-project-master/QGD/IndyHub Datasets/File29/svi_percent.xlsx'
#datad = f'{program_path}sherlock-project-master/QGD/IndyHub Datasets/File29/svi_desc.txt'
#cold = f'{program_path}sherlock-project-master/QGD/IndyHub Datasets/File29/svi_percent_dict.xlsx'

#Initializing files
filename = f'{program_path}sherlock-project-master/QGD/IndyHub Datasets/File30/savi_basic_needs_data.xlsx'
datad = f'{program_path}sherlock-project-master/QGD/IndyHub Datasets/File30/savi_basic_needs_desc.txt'
cold = f'{program_path}sherlock-project-master/QGD/IndyHub Datasets/File30/savi_basic_needs_data_dict.xlsx'
...
# Compare Column description to Keywords
#colcheck = list(columnd['Description']) 
...
filtered_columns = datatypes[datatypes['Data_Type'].isin(['address', 'birth Date', 'year','county','area'])]['Column']

meta_category_lists = {
    "date": ["birth Date", "day"],
    "loc": ["address", "area", "birth place", "city", "continent", "country", "location", "region"],
    "county": ["county"],
...
for index, row in combined_df.iterrows():
    if row['Original Meta Categories'] == 'year':
        ...
        if row['Data_Type'] == 'object':
            combined_df.at[index, 'Final MetaCategory'] = 'code'
            combined_df.at[index, 'Meta Category List'] = 'cat'
    elif row['Final MetaCategory'] in meta_category_lists['cat']:  # Check if in the "cat" list
        if row['Data_Type'] == 'numeric':
            combined_df.at[index, 'Final MetaCategory'] = 'class'
            combined_df.at[index, 'Meta Category List'] = 'num'
...
num_columns = len(cleandf.columns)
num_count = combined_df[combined_df['Meta Category List'].isin(['num'])].shape[0]
cat_count = combined_df[combined_df['Meta Category List'].isin(['cat'])].shape[0]
prf_count = combined_df[combined_df['Meta Category List'].isin(['prf'])].shape[0]
loc_count = combined_df[combined_df['Meta Category List'].isin(['loc'])].shape[0]
date_count = combined_df[combined_df['Meta Category List'].isin(['date'])].shape[0]
yr_count = combined_df[combined_df['Meta Category List'].isin(['year'])].shape[0]
output_data1 = [] 
output_data2 = []
#and num_count >= 1 and cat_count >= 1

if inp == 1:
    if (num_columns > 6 and num_count >= 1 and cat_count >= 1) or (num_columns > 10 and (num_count >= 1 or cat_count >= 1)):
    #if num_columns > 6 and num_count >= 1 and cat_count >= 1:
        print("Do you want to generate questions with combinations of 2 or 3 columns:")
        inpc = int(input("Enter either 2 or 3: "))
        if inpc == 2:
            g = 'y'
            output_data1 = [] 
            output_data2 = [] 
            ...
            while g == 'y' and question_count < 20:
                ...
                while counter in range(0, 5):
                    if question_count < 20:
                        ...
                        q1, que, para, que2, para2, best_version, sq = "", "", "", "", "", "", ""   # Initialize the variables
    ...
    # Save the initial DataFrame to an Excel file with folder name and timestamp included
    output_filename_initial1 = f"{program_path}sherlock-project-master/QGD/IndyHub Datasets/Main 11 Program Output/{folder_name}_questions_m11v2_{timestamp_str}.xlsx"
    output_df1.to_excel(output_filename_initial1, index=False)

    output_filename_initial2 = f"{program_path}sherlock-project-master/QGD/IndyHub Datasets/Main 11 Program Output/{folder_name}_questions_m11v3_{timestamp_str}.xlsx"
