...
#    operator_key = [key for key, value in operators.items() if sql[6] in value]
...
#        operator_key = operator_key[0]
    ...
    if (sql[6] == "Minimum"):
        ...
        if a1 == 1:
            ...
            sqtemp = "SELECT MIN('{}') FROM df WHERE '{}' == {}"
            sq = sqtemp.format(sql[5], sql[1], f'"{value1}"' if isinstance(value1, str) else value1)
            result = df[df[sql[1]] == value1][sql[5]].min()
            ...
            sqtemp = "SELECT '{}', MIN('{}') AS min FROM df GROUP BY '{}'"
            sq = sqtemp.format(sql[1], sql[5], sql[5], sql[1]) 
            result = df.groupby(sql[1])[sql[5]].min().reset_index(name='min')
        ...
        if a2 == 1:
            ...
            sqtemp = "SELECT MAX('{}') FROM df WHERE '{}' == {}"
            sq = sqtemp.format(sql[5], sql[1], f'"{value2}"' if isinstance(value2, str) else value2) 
            result = df[df[sql[1]] == value2][sql[5]].max()   
            ...
            que = template.format(des1, des2)
            que2 = template.format(des1_para, des2_para)
            sqtemp = "SELECT '{}', MAX('{}') AS max FROM df GROUP BY '{}'"
            sq = sqtemp.format(sql[1], sql[5], sql[1]) 
            result = df.groupby(sql[1])[sql[5]].min().reset_index(name='max')
        ...
        if a3 == 1:
            ...
            sqtemp = "SELECT '{}' FROM df GROUP BY '{}' HAVING AVG('{}') < {}"
            #sq = sqtemp.format(sql[1], sql[1], sql[5], (sql[7]))
            sq = sqtemp.format(sql[1], sql[1],  sql[5], f'"{sql[7]}"' if isinstance(sql[7], str) else sql[7]) 
            result = df.groupby(sql[1]).filter(lambda group: group[sql[5]].mean() < sql[7])
        ...
        template = "When {} is equal to {}, what are the total number of records for {}, within the dataset?"
        q1 = template.format(sql[1], str(value3), sql[5])
        que = template.format(des1, str(value3), des2)
        que2 = template.format(des1_para, str(value3), des2_para)
        sqtemp = "SELECT '{}', COUNT(*) FROM df WHERE '{}' = {} GROUP BY '{}'"
        sq = sqtemp.format(sql[5], sql[1], f'"{value3}"' if isinstance(value3, str) else value3, sql[5]) 
        result = df[df[sql[1]] == value3].groupby(sql[5]).size().reset_index(name='COUNT')
        ...
        sq = sqtemp.format(sql[1], sql[5], f'"{sql[7]}"' if isinstance(sql[7], str) else sql[7])  
        result = df[df[sql[5]] == sql[7]][sql[1]]  
