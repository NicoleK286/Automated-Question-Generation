...
import os
import pandas as pd
from cleaning import clean # cleaning file
from keywords import key   # keyword generation 
from dataprep import convert # dataprepartion 
from sher import sher        # sherlock code
from sql import sqlquery     #SQL query generation 
from questions import ques   # question generation 
from fips import fips        # fips code
import random
from Scraping import scraping
from paraphrases import *
...
#Initializing files
filename = '/N/u/ssjadhav/Carbonate/Sherlock/sherlock-project-master/QGD/example3/data.csv'
datad = '/N/u/ssjadhav/Carbonate/Sherlock/sherlock-project-master/QGD/example3/dataDescription.txt'
cold = '/N/u/ssjadhav/Carbonate/Sherlock/sherlock-project-master/QGD/example3/dataColumn.csv'
filepath, file_extension = os.path.splitext(filename)
...
if x == 0:
    ...
    cleandf = clean(df) # passing the df csv file through cleaning.py 
    #print(cleandf) # print the cleaned data.csv file
            ...
            for f in range(0,20):
                ...
                if counter <2:
                    if ((sql[1] == colcheck[maxind] or sql[5] == colcheck[maxind]) and (sql[6] == 'Minimum' or sql[6] == 'Maximum' or sql[6] == 'Average ' or sql[6] == 'COUNT' or sql[6] == 'sum of')):
                        qs = ques(sql,columnd,cleandf,sher_pred)
                        print(qs)
                        #result = print(type(qs))
                        #print(result)
                        #para = get_response(qs,1)
                        #print(para)
